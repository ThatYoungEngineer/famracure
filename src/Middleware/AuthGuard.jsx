...
const AuthGuard = ({ children }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Fetch user data only if authenticated but user data is null
        if (AuthUserData.isAuthenticated && get("TOKEN_USER") && !AuthUserData.user) {
    ...
    fetchUserData();
  }, [AuthUserData.isAuthenticated, AuthUserData.user, dispatch, navigate]);
  ...
  if (isLoading || (!AuthUserData.user && AuthUserData.isAuthenticated)) {
    return (
  ...
  if (
    ...
    get("TOKEN_USER") &&
    !AuthUserData.user?.email_verified_at
  ) {
